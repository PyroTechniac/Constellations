...
ava('chunk(too-short)', (test): void => {
	const entries = [0, 1, 2, 3, 4, 5];
	const chunkSize = 0;
	test.throws((): any => chunk(entries, chunkSize), { message: 'chunkSize must be 1 or greater.' });
});

ava('chunk(non-integer)', (test): void => {
	const entries = [0, 1, 2, 3, 4, 5];
	const chunkSize = 1.5;
	test.throws(() => chunk(entries, chunkSize), { message: 'chunkSize must be an integer.' });
});

ava('chunk(non-array)', (test): void => {
	const entries = {} as unknown as unknown[];
	const chunkSize = 2;
	test.throws(() => chunk(entries, chunkSize), { message: 'entries must be an array.' });
});
