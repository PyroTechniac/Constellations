...
ava('mergeDefault(interface)', (test): void => {
	interface Test {
		foo: string;
		bar: number;
		baz: boolean;
	...
	const a: Test = { foo: 'Hello, world!', bar: 42069, baz: true };
	const b: Partial<Test> = { foo: 'Goodbye, friends!', baz: false };

	const merged = mergeDefault(a, b);
	test.deepEqual(merged, { foo: 'Goodbye, friends!', bar: 42069, baz: false });
});
