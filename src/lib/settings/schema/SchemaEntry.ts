...
import { isNumber, isFunction } from '../../util/Utils';

export class SchemaEntry {

	/**
	 * The ConstellationClient for this SchemaEntry
	 */
	public client: ConstellationClient | null = null;
	...
	public constructor(parent: Schema | SchemaFolder, key: string, type: string, options: SchemaEntryOptions = {}) {
		...
		this.default = options.default ?? this._generateDefaultValue();
		this.minimum = options.minimum ?? null;
		this.maximum = options.maximum ?? null;
		this.inclusive = options.inclusive ?? false;
		this.configurable = options.configurable ?? this.type !== 'any';
		this.filter = options.filter ?? null;
		this.shouldResolve = options.resolve ?? true;
	}

	/**
	 * The serializer that manages this instance
	 */
	...
	/**
	 * The `JSON.stringify` behavior for this SchemaEntry
	 */
	...
	/**
	 * The default value generator, called when type and array are given but not the default value itself.
	 */
...
export interface SchemaEntryFilterFunction {
	(client: ConstellationClient, value: unknown, context: SerializerUpdateContext): boolean;
}
