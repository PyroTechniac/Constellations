...
export class Duration {
	...
	/**
	 * Gets the date from a specified time.
	...
	/**
	 * Defines the valueOf behavior of Duration.
	 */
	...
	/**
	 * The RegExp used for pattern parsing
	 */
	 ...
	 */
	public static toNow(earlier: TimeResolvable | Duration, showIn = false): string {
		if (earlier instanceof Duration) earlier = earlier.fromNow;
	...
	private static _parse(pattern: string): number {
		...
		pattern
			...
			.replace(this.aan, '1')
			// do math
			.replace(this.regex, (__, i, units): string => {
